...
class SecureTemplateRenderer:
    ...
    def _build_html_email(self, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.info(f"📊 Building email with {len(movies)} movies and {len(tv_shows)} TV shows")
    ...
    <div class="email-wrapper">
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
            ...
            if movies and len(movies) > 0:
                logger.info(f"🎬 Rendering {len(movies)} movies...")
                html_content += '''
                        <div class="section">
                ...
                for i, movie_item in enumerate(movies):
                    try:
                        logger.info(f"🎬 Rendering movie {i + 1}/{len(movies)}: {movie_item.get('title', 'Unknown')}")
                        movie_html = self._render_movie_item(movie_item)
                        html_content += movie_html
                        logger.info(f"✅ Successfully rendered movie {i + 1}")
                    except Exception as e:
                        logger.error(f"❌ Error rendering movie {i + 1} ({movie_item.get('title', 'Unknown')}): {e}")
                        continue

                html_content += '                        </div>'

            # TV Shows section - SIMPLIFIED FOR DEBUGGING
            if tv_shows and len(tv_shows) > 0:
                logger.info(f"📺 Starting TV shows section with {len(tv_shows)} shows...")

                # Simple section without calling _render_tv_show_item to isolate the error
                html_content += '''
                        <div class="section">
                            <div class="section-header">
                                <div class="section-icon">📺</div>
                ...
                # Add a simple placeholder for now to test
                html_content += '''
                            <div class="item">
                                <div class="item-content">
                                    <div class="item-title">TV Shows Placeholder</div>
                                    <div class="item-overview">TV shows rendering temporarily disabled for debugging</div>
                                </div>
                            </div>'''

                html_content += '                        </div>'
                logger.info("✅ TV shows section added (placeholder)")
            ...
            logger.error(f"❌ Error in _build_html_email: {e}")
        ...
        if tv_shows and len(tv_shows) > 0:
            ...
            for i, show_item in enumerate(tv_shows):
                try:
                    ...
                    logger.error(f"📍 Show data keys: {list(show_item.keys()) if isinstance(show_item, dict) else 'Not a dict'}")
...
Returns:
Dictionary
with total_movies_server and total_tv_shows_server
    """
try:
    import requests
...
Convenience
function
with manual URL and API key (legacy support).
